...
const BestSeller = () => {
     ...
     useEffect(() => {
       const fetchData = async () => {
         try {
           setIsLoading(true);
           const response = await axios.get(`https://openlibrary.org/search.json?q=bestseller&limit=10`);
                    ...
                    booksData && booksData.length > 0 && (
                         ...
                         >
                              {booksData.map((book , index) => (
                                   <Book key={index} showSave={false} title={book.title} authors={book.author_name} img={`http://covers.openlibrary.org/b/id/${book.cover_i}-M.jpg`} />
                              ))}
                         </Carousel>
                    )
               )}
